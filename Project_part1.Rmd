...
## Data Summary:
 This data is representative of a population of all UVA Football games from the 2024 season. It was collected during each game and uploaded to the UVA Athletics website. The third-down results were collected from the official UVA Athletics NCAA game statistics and box scores. As the NCAA tracks play-by-play outcomes, our data is recorded directly from the in-game tracking systems, which generate a full population of games for the season. The scoring totals and attendance numbers were also taken from this legitimate source. 

After downloading the data from the UVA Athletics webpage, no alterations to the original play-by-play data were made. The data were organized into simplified formats for the total game statistics, third-down attempts, and conversions. The clean, game-to-game summaries make comparisons easier when attempting to analyze the season’s results. After looking through the data, we chose only the data that we'd be using, that being the attendance, if a game is home or away, result of game, and score. All this data was in the first table in the full data set, and only had to find data for third down conversions for each game in the season, where we used the table set given and took the attempts, conversions, and conversion percentage. We modified their date format to make sure it was similar to all data sets then merged these data sets for use. 
...
```
## Conclusions: 
a. Interestingly, the away games have a higher scoring average, whereas the home games score less, as evidently shown by the box plot. The higher variability indicates that we are more unpredictable in how much we score in away games. Lastly, our home games seem to be more tightly clustered around an overall lower score count in games. 
...
In the 3rd down conversions, 4 games out of 12 do we have a better rate of 3rd down conversions compared to the other time. Of those 4, only 2 were above 15%. Of the 8 games where we had a worse conversion percent, 3 of the games were by more than 15%, with one being by 37.1%. There seems to be no improvement/worsening in conversion percent the later the season went. These summaries allowed us to directly compare home and away scoring, evaluate attendance’s influence, and analyze UVA’s third downs against their opponents. 

b. Notably the medians are over 10 points apart, which is a touchdown and a field goal worth of points. But, the variability in away games is slightly more, as depicted by a roughly 2.4 difference in IQR between the two. Lastly, the whiskers in the box plot are higher and lower for away games, showing that away games had a higher amount of points and the highest amount of points in their lowest scoring game. 
...
c. Our initial question addressed that there is more variability within away games, but surprisingly also higher scoring. This may show that there is more motivation for the players and a shift in mindset that occurs in AWAY games. 
...
Ray: ChatGPT 5 model was used for grammatical mistakes in our writing and analysis. 
