...
## Data Summary:
 (a) If the data represents a population, explain how the data were collected.  

This data is representative of a population of all UVA Football games from the 2024 season. It was collected during each game and uploaded to the UVA Athletics website. The third-down results were collected from the official UVA Athletics official NCAA game statitics and box scores. As NCAA tracks play--by-play outcomes our data is recorded directsly from the in game tracking systems, which generate a full populatio of games for the season.  

(c) Explain any data modifications after collection and the reasons behind these changes. 

After downloading the data from the UVA athletics webpage no alterations to the original play-by-play data were made. The data were organized into a simplified formats for the total game statistics, third-down attempts, and conversions. The clean game to game summaries make comparisons easier when attempting to analyze the seasons resultes.  

(d) Discuss potential issues with the data and their possible impact. 

Although the data is descriptive there are still uncaptured details of each game. First, injusries, weather, and field conditions are not included or accounted for. These confounding variables may play a role in the teams performance, but our data has no way of showing that. Secondly the data this project uses is only of the 2024 season. Meaning the results cannot be genaralized to team data of other years. 

(e) Explain why the data are appropriate to answer the research question. 

 This dataset includes the research question because they measure the variables in the research question. Third-down attempts within the data allow us to calculate conversion percentages so that we can compare performance across all games, and game outcomes. The metric of third-down efficiency is a meaningful metric, because drive plays a large role in football success. Since the data is complete, unbiased, and recorded using legitimate NCAA measurements, it is reliable for statistical analysis. 
